...
int main(int argc, char **argv) {
	ros::init(argc, argv, "RIA_BASE_controller");
	...
	ROS_INFO("[RIA_BASE_controller] Starting to spin...");
	...
	while (ros::ok()) {
		try {
			...
			ROS_ERROR("[RIA_BASE_controller] Exception: %s", e->what());
		} catch(...) {
		    ROS_ERROR("[RIA_BASE_controller] Unhandled exception");
        }
	}
